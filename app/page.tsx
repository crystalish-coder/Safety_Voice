...
export default function HomePage() {
  ...
  return (
    <div className="flex flex-col gap-10 pb-16">
      {/* Hero Section */}
      <section className="relative overflow-hidden border-b border-slate-200 bg-gradient-to-b from-blue-50/50 via-white to-slate-50 py-12 sm:py-16">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col items-center text-center">
            {/* Top Anonymity Badge */}
            <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-blue-200 bg-blue-100/60 px-4 py-1.5 text-xs font-semibold text-blue-800">
              <Lock className="h-3.5 w-3.5" />
              <span>100% 완전 익명 보장 | 신원 식별 정보 일체 미수집</span>
            </div>

            <h1 className="text-3xl font-extrabold tracking-tight text-slate-900 sm:text-4xl lg:text-5xl">
              <span className="text-blue-600">사내 안전 익명 제보 시스템</span>
            </h1>

            <p className="mt-4 max-w-2xl text-base sm:text-lg text-slate-600 leading-relaxed">
              위험 요소, 아차사고, 설비 이상, 보호구 부족 등 안전에 관한 모든 의견을
              신원 노출 부담 없이 편안하게 등록해 주세요. 안전관리팀이 신속히 확인하고 개선합니다.
            </p>
            ...
            <div className="mt-8 flex flex-wrap justify-center gap-3.5">
              <Link
                href="/board/new"
                className="flex items-center gap-2 rounded-xl bg-blue-600 px-6 py-3.5 text-base font-bold text-white shadow-md shadow-blue-500/20 transition hover:bg-blue-700 active:scale-[0.98]"
              >
      ...
      <div className="mx-auto w-full max-w-7xl space-y-12 px-4 sm:px-6 lg:px-8">
        ...
        <section>
          <div className="mb-4 flex items-center justify-between">
            ...
            <Link href="/board" className="text-xs font-semibold text-blue-600 hover:text-blue-700 flex items-center gap-1">
        ...
        {/* Recent Posts & Quick Guides Grid */}
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <Link href="/board" className="text-xs font-semibold text-blue-600 hover:text-blue-700">
                더보기
              </Link>
            </div>
            ...
            <div className="space-y-3">
              {isLoading ? (
                ...
                <div className="rounded-2xl border border-dashed border-slate-200 bg-white p-8 text-center space-y-2">
                  ...
                  <p className="text-sm font-bold text-slate-700">아직 등록된 안전 의견이 없습니다.</p>
                  <p className="text-xs text-slate-400">더 안전한 사업장을 위해 첫 번째 의견을 제보해 주세요!</p>
                  <div className="pt-2">
                    <Link
                      href="/board/new"
                      className="inline-flex items-center gap-1.5 rounded-lg bg-blue-600 px-3.5 py-2 text-xs font-bold text-white hover:bg-blue-700 transition"
                    >
                ...
                recentPosts.map((post) => {
                  ...
                  return (
                    <Link
                      key={post.id}
                      href={`/board/${post.id}`}
                      className="block rounded-xl border border-slate-200 bg-white p-4 sm:p-5 shadow-sm transition hover:border-blue-300 hover:shadow-md"
                    >
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        ...
                        <div className="flex items-center gap-1.5">
                          {riskInfo && (
                            <span className={`rounded-full border px-2.5 py-0.5 text-xs font-semibold ${riskInfo.badgeBg} ${riskInfo.color}`}>
                              위험도: {riskInfo.label}
                            </span>
                          )}
                          <span className={`rounded-full border px-2.5 py-0.5 text-xs font-semibold ${statusInfo.badgeBg} ${statusInfo.color}`}>
                            {statusInfo.label}
                          </span>
                        </div>
                      </div>

                      <h3 className="mt-2.5 text-base font-bold text-slate-900 line-clamp-1 group-hover:text-blue-600">
                        {post.title}
                      </h3>
